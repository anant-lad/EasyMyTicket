...
.DESCRIPTION
    ...
    creates a virtual environment, and registers scheduled tasks for
    the persistent WebSocket agent and daily health scan.
...
if (-not $ApiUrl) {
    $ApiUrl = Read-Host "EasyMyTicket backend URL (e.g. wss://api.example.com)"
}
if (-not $UserId) {
    $UserId = Read-Host "Your User ID (e.g. USR001)"
}
if (-not $ApiKey) {
    $ApiKey = Read-Host "Your Agent API Key (emt_xxxx)"
}

$InstallDir = "C:\EasyMyTicketAgent"
$CacheDir   = "C:\EasyMyTicketAgent\cache"
...
if (-not $PythonBin) {
    Write-Error "Could not install or locate Python 3. Please install it manually from https://www.python.org/downloads/"
    exit 1
}
...
& $VenvPython -m pip install --quiet "websockets>=12.0" "psutil>=5.9.0" "httpx>=0.25.0"

# ── Generate device UUID ──────────────────────────────────────────────────────
Write-Host "==> Generating device ID..."
$DeviceId = [System.Guid]::NewGuid().ToString()
$DeviceIdFile = "$CacheDir\device_id.txt"
Set-Content -Path $DeviceIdFile -Value $DeviceId -Encoding UTF8

# ── Create Windows Scheduled Task: persistent agent (runs at startup) ─────────
Write-Host "==> Registering scheduled task: EasyMyTicketAgent (startup)..."
$EnvBlock = @(
...
# Build environment variable list for the task
$TaskEnv = $EnvBlock | ForEach-Object {
    $parts = $_ -split "=", 2
    New-Object -TypeName PSObject -Property @{ Name = $parts[0]; Value = $parts[1] }
}

# Register the persistent agent task
$AgentTask = Register-ScheduledTask `
...
# Set environment variables on the task via XML (ScheduledTask API doesn't expose this directly)
$TaskXml = Export-ScheduledTask -TaskName "EasyMyTicketAgent" -TaskPath "\EasyMyTicket\"
$TaskXmlDoc = [xml]$TaskXml
foreach ($envEntry in $EnvBlock) {
    $parts = $envEntry -split "=", 2
    # We set these as system environment variables so the task inherits them
    [System.Environment]::SetEnvironmentVariable($parts[0], $parts[1], "Machine")
}

# ── Create Scheduled Task: daily scan at 06:00 ───────────────────────────────
Write-Host "==> Registering scheduled task: EasyMyTicketDailyScan (06:00)..."
...
# ── Start the agent now as a background job ───────────────────────────────────
Write-Host "==> Starting agent in background..."
Start-Job -ScriptBlock {
    param($py, $script)
    & $py $script
} -ArgumentList $VenvPython, $AgentScript | Out-Null

Write-Host ""
Write-Host "EasyMyTicket Agent installed successfully!"
Write-Host "  Device ID  : $DeviceId"
Write-Host "  API URL    : $ApiUrl"
Write-Host "  User ID    : $UserId"
Write-Host "  Install Dir: $InstallDir"
Write-Host ""
Write-Host "Task Scheduler entries:"
Write-Host "  \EasyMyTicket\EasyMyTicketAgent      (runs at startup)"
Write-Host "  \EasyMyTicket\EasyMyTicketDailyScan  (runs daily at 06:00)"
Write-Host ""
Write-Host "To view logs:"
Write-Host "  Get-Content $CacheDir\agent.log -Wait"
