...
# Embeds the full easymyticket_agent.py and sets up launchd agents.
#
# Usage:
#   bash install-macos.sh [--api-url wss://your-backend] [--user-id USR001] [--api-key emt_xxxx] [--auto-mode]
...
AGENT_SCRIPT="$INSTALL_DIR/easymyticket_agent.py"
...
cat > "$AGENT_SCRIPT" << 'PYTHON_SCRIPT'
"""
...
echo "==> Installing dependencies (websockets, psutil, httpx, ddgs)..."
"$INSTALL_DIR/.venv/bin/pip" install --quiet "websockets>=12.0" "psutil>=5.9.0" "httpx>=0.25.0" "ddgs>=9.0.0"
...
launchctl unload "$PLIST_DIR/${PLIST_WS}.plist"   2>/dev/null || true
launchctl unload "$PLIST_DIR/${PLIST_SCAN}.plist" 2>/dev/null || true

# -- Write LaunchAgent plist: persistent WebSocket connection
# KeepAlive=true: launchd auto-restarts the agent if it exits.
...
<dict>
    ...
    <dict>
        ...
        <key>AGENT_AUTO_MODE</key> <string>${AUTO_MODE}</string>
    </dict>
    <key>RunAtLoad</key>
    <true/>
    <key>KeepAlive</key>
...
# -- Write LaunchAgent plist: daily health scan at 06:00
...
if launchctl list | grep -q "$PLIST_WS"; then
    STATUS="Running"
else
    STATUS="Check logs: $CACHE_DIR/agent.log"
fi

# -- Summary
echo ""
echo "=============================================="
echo " EasyMyTicket Agent -- Installation Complete"
echo "=============================================="
echo "  Device ID   : $DEVICE_ID"
echo "  Backend URL : $API_URL"
echo "  User ID     : $USER_ID"
echo "  Install Dir : $INSTALL_DIR"
echo "  Agent       : $PLIST_WS ($STATUS)"
echo "  Daily Scan  : 06:00 via launchd"
echo ""
echo "Useful commands:"
echo "  launchctl list | grep easymyticket"
echo "  tail -f $CACHE_DIR/agent.log"
echo "  launchctl unload \"$PLIST_DIR/${PLIST_WS}.plist\"   # stop agent"
echo "  launchctl load   \"$PLIST_DIR/${PLIST_WS}.plist\"   # start agent"
echo ""
echo "The agent is running. Your device will appear in Agent Oversight within 30s."
