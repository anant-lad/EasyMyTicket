#!/usr/bin/env bash
# EasyMyTicket Agent — macOS Single-File Installer
# Embeds the full easymyticket_agent.py and sets up a LaunchAgent plist.
#
# Usage:
#   bash install-macos.sh [--api-url wss://your-backend] [--user-id USR001] [--api-key emt_xxxx]
#   Or interactive prompts if args not supplied.
...
if [[ -z "$API_URL" ]]; then
    read -rp "EasyMyTicket backend URL (e.g. wss://api.example.com): " API_URL
fi
if [[ -z "$USER_ID" ]]; then
    read -rp "Your User ID (e.g. USR001): " USER_ID
fi
if [[ -z "$API_KEY" ]]; then
    read -rp "Your Agent API Key (emt_xxxx): " API_KEY
fi

INSTALL_DIR="$HOME/Library/EasyMyTicketAgent"
CACHE_DIR="$HOME/Library/EasyMyTicketAgent/cache"
PLIST_DIR="$HOME/Library/LaunchAgents"
PLIST_WS="com.easymyticket.agent"
PLIST_SCAN="com.easymyticket.daily-scan"
...
mkdir -p "$INSTALL_DIR" "$CACHE_DIR" "$PLIST_DIR"

echo "==> Writing agent script..."
cat > "$HOME/Library/EasyMyTicketAgent/easymyticket_agent.py" << 'PYTHON_SCRIPT'
#!/usr/bin/env python3
"""
...
"$INSTALL_DIR/.venv/bin/pip" install --quiet "websockets>=12.0" "psutil>=5.9.0" "httpx>=0.25.0"

echo "==> Generating device ID..."
DEVICE_ID=$(python3 -c "import uuid; print(uuid.uuid4())")
echo "$DEVICE_ID" > "$CACHE_DIR/device_id.txt"
chmod 600 "$CACHE_DIR/device_id.txt"

VENV_PYTHON="$INSTALL_DIR/.venv/bin/python"

echo "==> Writing LaunchAgent plist (persistent WebSocket)..."
cat > "$PLIST_DIR/${PLIST_WS}.plist" << EOF
...
<dict>
    <key>Label</key>
    <string>${PLIST_WS}</string>
    <key>ProgramArguments</key>
    <array>
        <string>${VENV_PYTHON}</string>
        <string>${INSTALL_DIR}/easymyticket_agent.py</string>
    </array>
    <key>EnvironmentVariables</key>
    <dict>
        <key>AGENT_API_URL</key>  <string>${API_URL}</string>
        <key>AGENT_API_KEY</key>  <string>${API_KEY}</string>
        <key>AGENT_USER_ID</key>  <string>${USER_ID}</string>
        <key>AGENT_DEVICE_ID</key><string>${DEVICE_ID}</string>
        <key>AGENT_CACHE_DIR</key><string>${CACHE_DIR}</string>
    </dict>
    <key>RunAtLoad</key>
    <true/>
    <key>KeepAlive</key>
    ...
    <string>${CACHE_DIR}/agent.log</string>
    <key>StandardErrorPath</key>
    <string>${CACHE_DIR}/agent.err</string>
</dict>
</plist>
EOF

echo "==> Writing LaunchAgent plist (daily scan at 06:00)..."
cat > "$PLIST_DIR/${PLIST_SCAN}.plist" << EOF
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN"
  "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    ...
    <key>StandardOutPath</key>
    <string>${CACHE_DIR}/scan.log</string>
    <key>StandardErrorPath</key>
    <string>${CACHE_DIR}/scan.err</string>
</dict>
</plist>
EOF

echo "==> Loading LaunchAgents..."
launchctl load -w "$PLIST_DIR/${PLIST_WS}.plist"
launchctl load -w "$PLIST_DIR/${PLIST_SCAN}.plist"

echo ""
echo "EasyMyTicket Agent installed successfully!"
echo "  Device ID  : $DEVICE_ID"
echo "  API URL    : $API_URL"
echo "  User ID    : $USER_ID"
echo ""
echo "Useful commands:"
echo "  launchctl list | grep easymyticket"
echo "  tail -f $CACHE_DIR/agent.log"
echo "  launchctl unload $PLIST_DIR/${PLIST_WS}.plist  # to stop"
