#!/usr/bin/env bash
# EasyMyTicket Agent — Linux Single-File Installer
# Embeds the full easymyticket_agent.py and sets up a systemd service + daily timer.
#
# Usage:
#   sudo bash install-linux.sh [--api-url wss://your-backend] [--user-id USR001] [--api-key emt_xxxx]
...
cat > "/opt/easymyticket-agent/easymyticket_agent.py" << 'PYTHON_SCRIPT'
#!/usr/bin/env python3
"""
...
echo "==> Writing systemd service (persistent WebSocket)..."
...
Environment="AGENT_API_URL=${API_URL}"
...
ExecStart=${INSTALL_DIR}/.venv/bin/python ${INSTALL_DIR}/easymyticket_agent.py
Restart=always
RestartSec=10
...
echo "==> Writing systemd service (daily scan, one-shot)..."
...
Environment="AGENT_DEVICE_ID=${DEVICE_ID}"
Environment="AGENT_CACHE_DIR=${CACHE_DIR}"
ExecStart=${INSTALL_DIR}/.venv/bin/python ${INSTALL_DIR}/easymyticket_agent.py --scan
StandardOutput=journal
StandardError=journal
EOF

echo "==> Writing daily scan timer (06:00)..."
cat > "/etc/systemd/system/${SERVICE_SCAN}.timer" << EOF
[Unit]
Description=EasyMyTicket Daily Scan Timer
...
echo "  sudo systemctl status $SERVICE_WS"
echo "  sudo journalctl -u $SERVICE_WS -f"
echo "  sudo systemctl status ${SERVICE_SCAN}.timer"
