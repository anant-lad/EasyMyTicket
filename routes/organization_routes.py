...
@router.get("/organizations/{companyid}/analytics")
async def get_organization_analytics(companyid: str):
    """
    ...
    try:
        ...
        status_query = """
            ...
            GROUP BY status
            UNION ALL
            SELECT 'Resolved' as status, COUNT(*) as count FROM resolved_tickets WHERE companyid = %s
            UNION ALL
            SELECT 'Closed' as status, COUNT(*) as count FROM closed_tickets WHERE companyid = %s
        """
        status_data = db_conn.execute_query(status_query, (companyid, companyid, companyid))
        
        # 2. Tickets by Priority (Open only)
        priority_query = """
            ...
            GROUP BY priority
        """
        priority_data = db_conn.execute_query(priority_query, (companyid,))
        
        # 3. Tickets by Category (Open only)
        category_query = """
            SELECT ticketcategory, COUNT(*) as count 
            FROM new_tickets 
            WHERE companyid = %s 
            GROUP BY ticketcategory
        """
        category_data = db_conn.execute_query(category_query, (companyid,))
        
        # 4. Technician Performance (Resolved Tickets)
        tech_query = """
            SELECT t.tech_name, COUNT(rt.id) as resolved_count
            FROM technician_data t
            LEFT JOIN ticket_assignments ta ON t.tech_id = ta.tech_id
            LEFT JOIN resolved_tickets rt ON ta.ticket_number = rt.ticketnumber
            WHERE t.companyid = %s
            GROUP BY t.tech_name
            ORDER BY resolved_count DESC
            LIMIT 5
        """
        tech_perf_data = db_conn.execute_query(tech_query, (companyid,))
        
        # 5. Recent Activity (Last 5 tickets Created)
        recent_query = """
            SELECT ticketnumber, title, createdate, status
            FROM new_tickets
            WHERE companyid = %s
            ORDER BY createdate DESC
            LIMIT 5
        """
        recent_activity = db_conn.execute_query(recent_query, (companyid,))
        
        # Convert datetimes
        for activity in recent_activity:
            if isinstance(activity['createdate'], datetime):
                activity['createdate'] = activity['createdate'].isoformat()
        ...
        return {
            ...
            "analytics": {
                ...
                "top_technicians": tech_perf_data,
