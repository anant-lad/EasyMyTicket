...
from fastapi import APIRouter, HTTPException, Path, Query, File, UploadFile, Form, Depends
from src.database.db_connection import DatabaseConnection, AttachmentInfo
...
async def create_ticket(
    ...
    # Ideally these should be removed from Form and taken solely from token, 
    # but keeping them optional/ignored to maintain partial backward compatibility or 
    # we can strictly enforce token usage. Let's make them optional in Form but authoritative from Token.
    user_id: Optional[str] = Form(None, description="User ID (optional, overrides from token)"),
    companyid: Optional[str] = Form(None, description="Company ID (optional, overrides from token)"),
    ...
    """
    Create a new ticket using authenticated user credentials.
    ENFORCES: user_id and companyid from the JWT token.
    """
    # OVERRIDE: Trust the token, not the form data
    user_id = current_user['user_id']
    companyid = current_user['companyid']
    
    # Check for missing companyid in token (legacy users?)
    if not companyid:
         raise HTTPException(status_code=400, detail="User account is not associated with an Organization. Please contact support.")
    try:
...
async def get_all_tickets(
    ...
    ENFORCES: Tenant isolation based on authenticated user.
    """
    # Enforce Tenant Isolation
    # If user is NOT a super_admin, force companyid to match their token
    if current_user['role'] != 'super_admin':
        companyid = current_user['companyid']
        if not companyid:
             # Should not happen if token is valid, but safe guard
             return TicketsListResponse(success=True, tickets=[], total=0, limit=limit, offset=offset, has_more=False)
    else:
        # Super Admin can optionally filter by specific companyid, or see all if None
        pass
    try:
        db_conn = get_db_connection()
...
async def get_ticket(
    ticket_number: str = Path(..., description="The ticket number to retrieve"),
    current_user: Dict = Depends(get_current_user)
):
    """
    Get complete ticket details by ticket number.
    ENFORCES: Authorization check strictly against user's company ID.
...
async def get_ticket_by_id(
    ...
    try:
        ...
        # Enforce Tenant Isolation
        if current_user['role'] != 'super_admin':
            ticket_company_id = int(ticket.get('companyid')) if ticket.get('companyid') else None
            user_company_id = int(current_user['companyid']) if current_user['companyid'] else None
            
            if ticket_company_id != user_company_id:
                 raise HTTPException(
                    status_code=403,
...
async def update_ticket_status(
    ...
    try:
        ...
        # Validate ticket exists and get details for notification
        query = "SELECT ticketnumber, user_id, title, assigned_tech_id, status FROM new_tickets WHERE ticketnumber = %s"
        results = db_conn.execute_query(query, (ticket_number,))
        
        if not results:
            # Check other tables? For now assume active tickets
        ...
        # Notification Logic (Fail-safe)
        try:
            print(f"DEBUG: Checking notification triggers for {ticket_number}")
            print(f"DEBUG: User ID: {ticket.get('user_id')}, Old Status: {old_status}, New Status: {status_update.status}")
            
            # 1. Notify User of Status Change
            if ticket.get('user_id') and status_update.status != old_status:
                print("DEBUG: Triggering User Notification")
                notification_id = notification_service.create_notification(
                    recipient_id=ticket['user_id'],
                    title=f"Ticket Updated: {ticket_number}",
                    message=f"Status changed to '{status_update.status}' for ticket '{ticket['title']}'",
                    type="info",
                    related_entity_id=ticket_number,
                    related_entity_type="ticket"
                )
                print(f"DEBUG: Notification created with ID: {notification_id}")

            # 2. Notify Technician of Assignment
            if status_update.tech_id and status_update.tech_id != ticket.get('assigned_tech_id'):
                 print("DEBUG: Triggering Technician Notification")
                 notification_service.create_notification(
                    recipient_id=status_update.tech_id,
                    title="New Ticket Assigned",
                    message=f"You have been assigned ticket {ticket_number}: '{ticket['title']}'",
                    type="warning", # or info
                    related_entity_id=ticket_number,
                    related_entity_type="ticket"
                )
        except Exception as notify_error:
            print(f"WARNING: Notification failed for ticket {ticket_number}: {notify_error}")
            import traceback
            traceback.print_exc()
...
async def post_ticket_message(ticket_number: str, request: CommunicationRequest):
    ...
    try:
        ...
        try:
             # Check new_tickets first
             res = db_conn.execute_query("SELECT user_id, assigned_tech_id, title FROM new_tickets WHERE ticketnumber = %s", (ticket_number,))
             if not res:
                  # Check resolved/closed if commenting on old tickets is allowed
                  res = db_conn.execute_query("SELECT user_id, assigned_tech_id, title FROM closed_tickets WHERE ticketnumber = %s", (ticket_number,))
             
             if res:
                 ticket = res[0]
                 
                 # Case 1: Tech replied -> Notify User
                 if request.sender_type == 'technician' and ticket.get('user_id'):
                      notification_service.create_notification(
                        recipient_id=ticket['user_id'],
                        title=f"New Reply: {ticket_number}",
                        message=f"Technician replied: {request.message_text[:50]}...",
                        type="info",
                        related_entity_id=ticket_number,
                        related_entity_type="ticket"
                    )
                 
                 # Case 2: User replied -> Notify Assigned Tech
                 elif request.sender_type == 'user' and ticket.get('assigned_tech_id'):
                      notification_service.create_notification(
                        recipient_id=ticket['assigned_tech_id'],
                        title=f"New User Reply: {ticket_number}",
                        message=f"User replied: {request.message_text[:50]}...",
                        type="info",
                        related_entity_id=ticket_number,
                        related_entity_type="ticket"
                    )
        except Exception as notify_error:
            print(f"Failed to send notification for message: {notify_error}")
