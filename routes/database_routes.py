...
@router.delete("/database/users/{user_id}", response_model=GenericResponse)
async def delete_user(
    user_id: str = Path(..., description="User ID to delete"),
    companyid: str = Query(..., description="Organization ID for security check")
):
    """
    Delete a user from the database. 
    Requires companyid to match the user's organization.
    """
    try:
        db_conn = get_db_connection()
        
        # Verify user belongs to company
        check_query = "SELECT companyid FROM user_data WHERE user_id = %s"
        result = db_conn.execute_query(check_query, (user_id,))
        
        if not result:
             raise HTTPException(status_code=404, detail="User not found")
             
        if result[0]['companyid'] != companyid:
             raise HTTPException(status_code=403, detail="Permission denied: User belongs to different organization")
        
        # Delete user
        delete_query = "DELETE FROM user_data WHERE user_id = %s"
        db_conn.execute_query(delete_query, (user_id,), fetch=False)
...
@router.delete("/database/technicians/{tech_id}", response_model=GenericResponse)
async def delete_technician(
    tech_id: str = Path(..., description="Technician ID to delete"),
    companyid: str = Query(..., description="Organization ID for security check")
):
    """
    Delete a technician from the database.
    Requires companyid to match the technician's organization.
    """
    try:
        db_conn = get_db_connection()
        
        # Verify tech belongs to company
        check_query = "SELECT companyid FROM technician_data WHERE tech_id = %s"
        result = db_conn.execute_query(check_query, (tech_id,))
        
        if not result:
             raise HTTPException(status_code=404, detail="Technician not found")
             
        if result[0]['companyid'] != companyid:
             raise HTTPException(status_code=403, detail="Permission denied: Technician belongs to different organization")
        
        # Delete technician
        delete_query = "DELETE FROM technician_data WHERE tech_id = %s"
        db_conn.execute_query(delete_query, (tech_id,), fetch=False)
        
        return GenericResponse(
            success=True,
            message=f"Technician {tech_id} deleted successfully"
        )
    except HTTPException:
        raise
    except Exception as e:
